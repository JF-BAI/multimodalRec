...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model, mg=False):
        ...
        self.item_topk=config["item_topk"]
    ...
    def plot_train_loss(self, show=True, save_path='/home/bjf/bjf_projects/MMRec/src/loss_graph'):
        r"""Plot the train loss in each epoch

        Args:
            show (bool, optional): whether to show this figure, default: True show (bool, 可选): 是否显示此图像，默认为 True。
            save_path (str, optional): the data path to save the figure, default: None.保存图像的数据路径，默认为 None。如果为 None，则不会保存图像。
                                       If it's None, it will not be saved.
        """
        epochs = list(self.train_loss_dict.keys())# 获取所有记录了训练损失的epoch，并将其转换为列表
        epochs.sort()# 对epoch进行排序，以确保按照正确的顺序绘制
        values = [float(self.train_loss_dict[epoch]) for epoch in epochs]# 从字典中提取每个epoch对应的损失值，并转换为浮点数列表
        # 使用matplotlib绘制epoch与损失的关系图
        plt.plot(epochs, values)# 使用matplotlib绘制epoch与损失的关系图
        plt.xticks(epochs)# 设置x轴刻度为所有的epoch值
        plt.xlabel('Epoch')# 设置x轴标签为"Epoch"
        plt.ylabel('Loss')# 设置y轴标签为"Loss"
        if show:# 根据show参数决定是否显示图像
            plt.show()
        if save_path:# 根据save_path参数决定是否保存图像
