...
class TrainDataLoader(AbstractDataLoader):
    ...
    def inter_matrix(self, form='coo', value_field=None):
        """获取描述用户ID和项目ID之间交互的稀疏矩阵。

        稀疏矩阵的形状为(user_num, item_num)。

        对于一行<src, tgt>，如果`value_field`为`None`，则`matrix[src, tgt] = 1`，
        否则`matrix[src, tgt] = self.inter_feat[src, tgt]`。

        参数:
            form (str, optional): 稀疏矩阵格式。默认为`coo`。
            value_field (str, optional): 稀疏矩阵的数据，应该存在于`df_feat`中。
                默认为`None`。

        返回:
            scipy.sparse: 格式为`coo`或`csr`的稀疏矩阵。
        """
        if not self.dataset.uid_field or not self.dataset.iid_field:# 检查数据集是否包含用户ID和项目ID字段
            raise ValueError('dataset doesn\'t exist uid/iid, thus can not converted to sparse matrix')
        return self._create_sparse_matrix(self.dataset.df, self.dataset.uid_field,# 创建并返回稀疏矩阵
