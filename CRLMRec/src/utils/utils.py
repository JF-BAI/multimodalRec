...
def get_model(model_name):
    ...
    module_path = '.'.join(['models', model_file_name])# 构建模块的完整路径
    print("________________",module_path)
    if importlib.util.find_spec(module_path, __name__):# 检查模块路径是否可导入
        model_module = importlib.import_module(module_path, __name__)# 如果模块存在，则导入该模块

    model_class = getattr(model_module, model_name)# 通过模块名称获取模型类
    return model_class
